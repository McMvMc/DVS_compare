...
from pytorch3d.structures import Pointclouds  # needed for using p3d.transforms
...
def train_DDP(rank):
    ...
    print(f"CUDA_VISIBLE_DEVICES is {rank}")
    ...
    decay_iteration = max(25, n_frames)
    ...
    for i in range(start, N_iters):
        ...
        for serial in args.serial_list:
            ...
            # Move training data to GPU
            images = all_images[serial]  # .to(device)
            invdepths = all_depths[serial]  # .to(device)
            masks = all_masks[serial]
            poses = all_poses[serial]
            # render_poses = all_render_poses[serial]
            # bds = all_bds[serial]
            # ref_c2w = all_ref_c2w[serial]
            grids = all_grids[serial]
            dist_mask = all_dist_masks[serial]
            ...
            # No raybatching as we need to take random rays from one image at a time
            t = img_i / n_frames * 2. - 1.0 # time of the current frame
            target = images[img_i].to(rank)
            pose = poses[img_i, :3, :4]
            hwf = poses[0, :3, -1]
            H, W, focal = hwf
            H, W = int(H), int(W)
            mask = masks[img_i].to(rank) # Static region mask
            invdepth = invdepths[img_i].to(rank)
            grid = grids[img_i].to(rank)

            rays_o, rays_d = get_rays(H, W, focal, torch.Tensor(pose.cpu()).to(rank)) # (H, W, 3), (H, W, 3)
            ...
            rays_o = select_batch(rays_o).to(rank) # (N_rand, 3)
            rays_d = select_batch(rays_d).to(rank) # (N_rand, 3)
            target_rgb = select_batch(target).to(rank)
            batch_grid = select_batch(grid).to(rank) # (N_rand, 8)
            batch_mask = select_batch(mask[..., None]).to(rank)
            batch_dist_mask = select_batch(dist_mask[..., None]).to(rank)
            batch_invdepth = select_batch(invdepth).to(rank)
            batch_rays = torch.stack([rays_o, rays_d], 0).to(rank)
            ...
            img_loss = img2mse(ret['rgb_map_full'], target_rgb, batch_dist_mask)
            ...
            # Compute MSE loss between rgb_s and true RGB.
            img_s_loss = img2mse(ret['rgb_map_s'], target_rgb, batch_mask * batch_dist_mask)
            psnr_s = mse2psnr(img_s_loss.cpu())
            ...
            img_d_loss = img2mse(ret['rgb_map_d'], target_rgb, batch_dist_mask)
            psnr_d = mse2psnr(img_d_loss.cpu())
            ...
            img_d_f_loss = img2mse(ret['rgb_map_d_f'], target_rgb, batch_dist_mask)
            psnr_d_f = mse2psnr(img_d_f_loss.cpu())
            ...
            img_d_b_loss = img2mse(ret['rgb_map_d_b'], target_rgb, batch_dist_mask)
            psnr_d_b = mse2psnr(img_d_b_loss.cpu())
            ...
            depth_loss = compute_depth_loss(ret['depth_map_d'] * batch_dist_mask[:,0], -batch_invdepth * batch_dist_mask[:,0])
        ...
        if i % args.i_video == 0 and i>0 and rank == 0:
            ...
            # save_res(moviebase, ret)
            n_frame_each_edge = 5
            save_dir = os.path.join(args.datadir, f'free_view_{args.start_frame}to{args.end_frame}'
                                                  f'_{args.target_idx}_{i}')
        ...
        if i % args.i_img == 0:
            ...
            target = images[img_i].to(rank)
            pose = poses[img_i, :3, :4].to(rank)
            mask = masks[img_i].to(rank)
            grid = grids[img_i].to(rank)
            invdepth = invdepths[img_i].to(rank)
            ...
            with torch.no_grad():
                ...
                # The last frame does not have forward flow.
                pose_f = poses[min(img_i + 1, int(n_frames) - 1), :3, :4]
                ...
                psnr = mse2psnr(img2mse(ret['rgb_map_full'], target).cpu())
